...
echo "⏳ Ожидаем полной готовности Jenkins..."
for i in {1..30}; do
    if curl -s -u admin:${JENKINS_PASSWORD} http://localhost:8080/my_jenkins/api/json > /dev/null 2>&1; then
        echo "✅ Jenkins готов"
        break
    fi
    echo "Ждём... ($i/30)"
...
declare -A JOBS=(
    ["UI-Tests-Selenoid"]="https://github.com/dimamoryk/AutotestWithSolenoid.git"
